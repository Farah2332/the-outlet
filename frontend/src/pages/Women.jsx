...
const Women = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                const data = await getProductsBySection("women", categoryMap[category]);
        ...
        <div>
            ...
            <Nav className="bg-dark text-white py-2 justify-content-center">
                <Container className="d-flex justify-content-center">
                    {["shirts", "pants", "dresses", "shoes"].map((item) => (
                        <Nav.Item key={item}>
                            <Nav.Link
                                as={Link}
                                to={`?category=${item}`}
            ...
            <Container className="py-4">
                <h1 className="text-center">Women's Collection - {category.toUpperCase()}</h1>
                ...
                <div className="row g-4">
                    {products.length > 0 ? (
                        products.map((product) => {
                            if (!product.colors || product.colors.length === 0) {
                                return null; // Skip products without colors
                            }
                            ...
                            return (
                                <div key={product.id} className="col-12 col-sm-6 col-md-4 col-lg-3">
                                    <div className="card h-100 d-flex flex-column">
                                        ...
                                        <img
                                            src={selectedColorForProduct ?.image || product.colors[0] ?.image || "https://placehold.co/150"}
                                            alt={product.name}
